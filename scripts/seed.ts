...
import { sectionTitleFrom, serializeMarkdown } from "../shared/journal.ts";
...
function entry({ prefix, at, title = null, tags, task = null, body }: { prefix: string; at: string; title?: string | null; tags: string[]; task?: string | null; body: string }): JournalEntry {
  return { id: identifier(prefix), at, sectionTitle: sectionTitleFrom(at), displayTitle: title, tags, task, body };
}

const journal: JournalEntry[] = [
  entry({ prefix: "jnl", at: stamp(-14, "08:15:00"), tags: ["personal"], body: "Quiet morning. Coffee on the step. The neighbour's cat came over and sat on the warm stone." }),
  entry({
    prefix: "jnl",
  ...
  entry({ prefix: "jnl", at: stamp(-9, "19:30:00"), tags: ["exercise", "climbing"], body: "Hangboard felt heavy. Kept the 60s rest and it was fine by the third set." }),
  entry({
    prefix: "jnl",
    at: stamp(-7, "07:42:00"),
    title: "slept badly",
  ...
  entry({ prefix: "jnl", at: stamp(-5, "18:05:00"), title: "blue v4", tags: ["climbing"], body: "Sent the blue V4 on the overhang. Heel hook first, then trust the left hand." }),
  entry({ prefix: "jnl", at: stamp(-3, "21:10:00"), tags: ["therapy", "personal"], body: "Walked before dinner. Fewer tabs open in my head." }),
  entry({
    prefix: "jnl",
    at: stamp(-1, "07:40:00"),
    tags: ["personal"],
...
const notebook: JournalEntry[] = [
  entry({ prefix: "nte", at: stamp(-20, "15:15:00"), title: "the overstory", tags: ["books"], body: "Sam recommended The Overstory. Library has it, hold placed." }),
  entry({
    prefix: "nte",
  ...
  entry({ prefix: "nte", at: stamp(-4, "13:30:00"), title: "books to find", tags: ["books"], body: "Anything else by Powers. The Le Guin essays. The short one about walking that Ana mentioned." }),
  entry({
    prefix: "nte",
    at: stamp(-2, "18:20:00"),
    title: "water filter",
  ...
  entry({ prefix: "nte", at: stamp(0, "09:30:00"), title: "rosemary", tags: ["garden"], body: "The rosemary wants the sunny bed by the fence. Move it before the frost." }),
];
